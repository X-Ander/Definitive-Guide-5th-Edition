...
Asterisk построен на модулях. Модуль — это загружаемый компонент, который обеспечивает определенную функциональность. Это может быть драйвер какого-нибудь канала (например, `chan_pjsip.so`) или ресурс, позволяющий подключиться к внешней технологии (например, `func_odbc.so`). Модули Asterisk'а загружаются на основе параметров, определенных в файле _/etc/asterisk/modules.conf_. В этой книге мы обсудим использование многих модулей, а сейчас просто введём саму концепцию модулей и дадим представление об их доступных типах.

На самом деле можно запустить Asterisk вообще без каких-либо модулей, хотя в этом состоянии он ничего не сможет сделать. Это полезно для понимания модульной природы Asterisk'а, чтобы оценить его архитектуру.

<table border="1" width="100%" cellpadding="5">
  <tr>
    <td>
    <p><img src="pics/note.png" height="100" align="left">Можно запустить Asterisk без модулей, загружаемых по умолчанию, и загружать каждый нужный модуль вручную из консоли, но это явно не то, что стоит запускать в работу. Единственный случай, когда это было бы полезно — тонкая настройка производительности системы, при которой хочется убрать все, что не требуется для конкретного применения Asterisk'а.</p>
    </td>
  </tr>
</table>
