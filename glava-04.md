...
Если вы заинтересованы в безопасной связи через общедоступный интернет (а вы заинтересованы, уж поверьте), то полезно иметь доменные сертификаты, предоставленные центром сертификации (ЦС).

Проект LetsEncrypt предоставляет бесплатные цифровые сертификаты проверки домена (Domain Validation — DV). Бесплатный инструмент, предоставляемый Let's Encrypt Foundation под названием `certbot`, позволяет автоматизировать получение и обслуживание доверенных сертификатов.

Вашему серверу как минимум потребуется полное доменное имя ([FQDN](https://ru.wikipedia.org/wiki/FQDN)), которое соответствует IP-адресу, на который эта машина принимает внешние соединения. Любой промежуточный брандмауэр должен будет передавать трафик для этого имени в систему, для которой вы получаете сертификат. Если вы по какой-то причине не можете это сделать, получить доверенный сертификат будет сложнее \(и это выходит за рамки книги\).
...
После установки нужно просто выполнить следующее:
